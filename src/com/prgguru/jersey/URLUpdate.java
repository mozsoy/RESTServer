...
/*	@GET
    ...
    @Produces(MediaType.APPLICATION_JSON) 
    // Query parameters are parameters: http://localhost/<appln-folder-name>/urlupdate/doupdate?user_id=abc&urldata=xyz
    public String doUpdate(@QueryParam("user_id") String uid, @QueryParam("urldata") HashMap<String,Integer> urldata){
        String response = "";
        if(checkCredentials(uid)){
        	System.out.println(urldata);
        	if(updateUrl(urldata) == 0){
    ...
    */
    ...
    private int updateUrl(HashMap<String, Integer> urlMap) {
    	int result = -1;
    	try {
			JSONObject jUrlData = new JSONObject(urlMap);
			if(DBConnection.insertUrl(jUrlData)) {
